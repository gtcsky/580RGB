	...
	*REG_IO_CONTROL = 0x00;
	for (i_channel = 2; i_channel < (ADC_CH_NUM - 1); i_channel++) {
		if (channel & BIT(i_channel)) {
			...
			set_sampling_resolution(a_channel, (is_high_resolution & BIT(i_channel)), (is_differential_mode & BIT(i_channel)));
		}
	}
}
...
bool	 getAdcStts(void){
	return mAdc_Ctx.enable;
}

void	resetAdcParams(void){
	clear_adcc_cfg();
}
...
uint16 usbAbsSubUint16(uint16 d1,uint16 d2){
...
float hal_adc_value_cal(adc_CH_t ch, uint16_t* buf, uint32_t size, uint8_t high_resol, uint8_t diff_mode)
{
    ...
    {
        adc_sum += (buf[i] & 0xfff);
//        if(ADC_CHANNEL_TEMP==ch){
//        	LOG("\n buff[%d]=0x%03x \n",i,buf[i] & 0xfff);
//        }
    }
        if(ADC_CHANNEL_TEMP==ch||ADC_CHANNEL_BATT==ch){
        	if(usbAbsSubUint16((buf[8] & 0xfff),(buf[size-1] & 0xfff))>100){
        	//	LOG("\n __GAP=%d",usbAbsSubUint16((buf[8] & 0xfff),(buf[size-1] & 0xfff)));
        		return -1.0;
        	}
        }
    result = ((float)adc_sum) / (size-8);
