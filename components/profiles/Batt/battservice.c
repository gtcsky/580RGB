...
// Battery Power characteristic
CONST uint8 battPowerUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(BATT_POWER_UUID), HI_UINT16(BATT_POWER_UUID)
};
CONST uint8 battPowerCompUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(BATT_LEVEL_STTS_UUID), HI_UINT16(BATT_LEVEL_STTS_UUID)
};
...
static uint8 battCompensationProps = GATT_PROP_READ | GATT_PROP_NOTIFY;
static uint8 battCompensationVolt = 0;
...
static gattCharCfg_t battCompensationCharCfg[GATT_MAX_NUM_CONN];
...
//        { ATT_BT_UUID_SIZE, battLevelUUID },
        { ATT_BT_UUID_SIZE, battPowerUUID },
        GATT_PERMIT_READ,
        0,
        &battLevel
      },

      // 3 Battery Level Client Characteristic Configuration
      {
    ...
    // 4 Battery Level Declaration
    {
      { ATT_BT_UUID_SIZE, characterUUID },
      GATT_PERMIT_READ,
      0,
      &battCompensationProps
    },

      // 5 Battery Level Value
      {
//        { ATT_BT_UUID_SIZE, battLevelUUID },
        { ATT_BT_UUID_SIZE, battPowerCompUUID },
        GATT_PERMIT_READ,
        0,
        &battCompensationVolt
      },

      // 6 Battery Level Client Characteristic Configuration
      {
        { ATT_BT_UUID_SIZE, clientCharCfgUUID },
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        (uint8 *) &battCompensationCharCfg
      },

      // 7 HID Report Reference characteristic descriptor, batter level input
      {
...
static uint8 compensationVoltMeasure(void);
/*********************************************************************
 * PROFILE CALLBACKS
 */
...
//		}
		*pLen = 5;
		pValue[0] = battLevel;
		temp=(uint16)(BATT_VOLT_MIN*100);
		pValue[1] =temp/256;
		pValue[2] =temp%256;
		temp=(uint16)(BATT_VOLTAGE_MAX/10);
		pValue[3] =temp/256;
		pValue[4] =temp%256;
	}else if (uuid == BATT_LEVEL_STTS_UUID) {
		uint8 level;
		uint16 temp;
		level = compensationVoltMeasure();
		battLevel = level;
		*pLen = 5;
		pValue[0] = battLevel;
		temp=(uint16)(BATT_VOLT_MIN*100);
 ...
 * @return  Success or Failure
 */
static bStatus_t battWriteAttrCB(uint16 connHandle, gattAttribute_t *pAttr, uint8 *pValue, uint16 len, uint16 offset) {
	bStatus_t status = SUCCESS;
...
static uint8 battMeasure(void) {
	...
	percent=batt_get_percent();


	percent=(uint8)(((BATT_VOLTAGE_MAX-BATT_VOLTAGE_MIX)*percent/100)/10);
	return percent;
}

static uint8 compensationVoltMeasure(void) {
	uint8 volt = 0;
	volt = (uint8) ((getCompensationVolt() * 1000 - BATT_VOLTAGE_MIX) / 10);
	return volt;
}
/*********************************************************************
