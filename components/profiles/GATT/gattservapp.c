...
extern	gattCharCfg_t simpleProfileChar4Config[GATT_MAX_NUM_CONN];
extern	gattCharCfg_t simpleProfileChar6Config[GATT_MAX_NUM_CONN];
...
static bStatus_t gattServApp_ProcessFindByTypeValueReq(gattMsgEvent_t* pMsg, uint16* pErrHandle)
{
	...
	{
		uint16 grpEndHandle;
//    LOG("FindByTypeValueReq@gattservapp.c \n");
...
//    LOG("ReadByTypeReq@gattservapp.c \n");
...
static bStatus_t gattServApp_ProcessReadReq(gattMsgEvent_t* pMsg, uint16* pErrHandle)
{
	...
	if(pAttr != NULL)
	{
		...
		// in this response.
//        LOG("ProcessReadReq@gattservapp.c \n");
...
//    LOG("ReadMultiReq@gattservapp.c \n");
...
//    LOG("ReadByGrpTypeReq@gattservapp.c \n");
		status = GATTServApp_ReadAttr(pMsg->connHandle, pAttr, service, attrValue,
                           ...
                           uint16 offset, uint8 maxLen)
{
	uint8 useCB = FALSE;
	bStatus_t status = SUCCESS;
//	LOG("GATTServApp_ReadAttr@gattservapp.c\n");
	// Authorization is handled by the application/profile
...
{
//	LOG("SendServiceChangedInd@gattservapp.c\n");
                               ...
                               uint16 value)
{
	...
	if(pItem == NULL)
	{
//		LOG("WriteCharCfg@gattservapp.c");
...
{
	bStatus_t status = SUCCESS;
//	LOG("ProcessCCCWriteReq@gattservapp.c\n");
	// Validate the value
	if(offset == 0)
	{
		if(len == 2)
                                     ...
                                     uint16 numAttrs, uint8 taskId)
{
	...
	for(uint8 i = 0; i < GATT_MAX_NUM_CONN; i++)
	{
		...
		if((pItem->connHandle != INVALID_CONNHANDLE) &&
				(pItem->value != GATT_CFG_NO_OPERATION)){
//	if(charCfgTbl==simpleProfileChar4Config) {
//		LOG(" char4 info %d@gattservapp.c\n",i);
//	} else if(charCfgTbl==simpleProfileChar6Config) {
//		LOG(" char6 info %d@gattservapp.c\n",i);
//	}else{
//		LOG(" other info %d@gattservapp.c\n",i);
//	}
			gattAttribute_t* pAttr;
			...
			if(pAttr != NULL){
				...
				if(GATTServApp_ReadAttr(pItem->connHandle, pAttr,GATT_SERVICE_HANDLE(attrTbl), noti.value,
