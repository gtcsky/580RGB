...
/**************************************************************************************************
  Filename:       OSAL_bleSmartPeripheral.c
  Revised:
  Revision:
...
const pTaskEventHandlerFn tasksArr[] =
{
    ...
    bleSmartPeripheral_ProcessEvent,				// task 8
	protocol_uart_ProcessEvent,						// task 9
	light_ProcessEvent,								// tash 10
    command_center_ProcessEvent,					// task 11
    userWatchDogProcessEvent,
};
...
void osalInitTasks(void) {
	...
	/* Flash */
	systems_parameters_Init();

	/* Application */
	bleSmartPeripheral_Init(taskID++);
	protocol_uart_Init(taskID++);
	light_Init(taskID++);
	command_center_Init(taskID++);
	userWatchDogInit(taskID);
}
